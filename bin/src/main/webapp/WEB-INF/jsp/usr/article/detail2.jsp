...
<div class="list-container">
	...
	<div class="list-items-container">
		<div class="list-items-section">
			...
			<img src="https://attachment.hibrain.io/recruits/3469412/1713143906596/제목 없음.png"
				style="height: 1234px; width: 900px;">
			import * as React from "react"; interface FavoriteButtonProps { imageUrl: string; text: string; } const
			FavoriteButton: React.FC
			<FavoriteButtonProps> = ({ imageUrl, text }) => { return (
			<div
				className="overflow-hidden relative flex-col justify-center items-start self-end px-5 py-2.5 mt-10 w-40 text-sm tracking-tighter leading-3 whitespace-nowrap aspect-[5.52] text-neutral-700 max-md:pr-5">
				<img src={imageUrl} alt="" className="object-cover absolute inset-0 size-full" />
				{text}
			</div>
			); }; interface InfoRowProps { label: string; value: string; } const InfoRow: React.FC<InfoRowProps> =
			({ label, value }) => { return ( <>
			<div
				className="justify-center items-start p-3 whitespace-nowrap border-t border-b border-solid bg-neutral-100 border-neutral-200 max-md:pr-5">
				{label}</div>
			<div
				className="justify-center items-start px-5 py-3.5 border-t border-b border-solid border-neutral-200 leading-[160%] text-neutral-700">
				{value}</div>
			</> ); }; function MyComponent() { const infoRows = [ { label: "접수기간", value: "24.04.26 13:30 ~ 24.04.26 17:30" }, {
			label: "관련 홈페이지", value: "바로가기" }, { label: "총시상금", value: "무료" }, { label: "1등 시상금", value: "공고문참고" }, { label: "담당자
			연락처", value: "031-759-9934" }, { label: "담당자 이메일", value: "overseas@kntsc.kr" }, ]; return (
			<div className="flex flex-col">
				<header
					className="flex gap-5 justify-between px-5 w-full text-lg font-bold tracking-tighter whitespace-nowrap text-neutral-700 max-md:flex-wrap max-md:max-w-full">
					<h1>학술행사</h1>
					<img
						...
						alt="" loading="lazy" className="shrink-0 self-start mt-1 w-52 max-w-full aspect-[11.11]" />
				</header>
				<section className="flex flex-col pt-7 mt-2.5 w-full border-t-2 border-orange-300 border-solid max-md:max-w-full">
					<div className="flex gap-5 justify-between px-px w-full max-md:flex-wrap max-md:max-w-full">
						<article className="flex flex-col px-5">
							<h2 className="text-2xl font-bold tracking-tighter leading-7 text-zinc-900">한국비임상기술지원센터 웨비나</h2>
					...
					<div
						className="flex flex-col justify-center w-full text-sm tracking-tighter leading-5 text-neutral-700 max-md:max-w-full">
						{infoRows.map(({ label, value }, index) => (
						<div key={index} className="flex gap-0 justify-center pr-20 w-full max-md:flex-wrap max-md:pr-5 max-md:max-w-full">
				...
				<div
					className="flex justify-center items-center px-16 py-5 w-full tracking-tighter text-center border-t border-b border-solid bg-neutral-100 border-neutral-200 max-md:px-5 max-md:max-w-full">
					<div className="flex gap-5 max-md:flex-wrap">
						<div className="flex gap-1.5 text-sm font-bold leading-6 text-red-400 whitespace-nowrap">
							<img
								...
								alt="" loading="lazy" className="shrink-0 w-5 aspect-square" />
						...
						<div className="flex-auto text-base leading-6 text-zinc-800 max-md:max-w-full">
							...
							<span className="text-red-400"> 반드시 다음 원본 사이트에서 확인후 지원바랍니다. </span>
							{" "}
							<span className="font-bold text-zinc-800"> 원본사이트 바로가기 &gt; </span>
						</div>
					</div>
				</div>
				<FavoriteButton
				...
				<footer
					className="justify-center px-5 py-3.5 w-full text-lg font-bold tracking-tighter text-black whitespace-nowrap border-b border-solid border-neutral-600 max-md:max-w-full">
					장소/교통편 </footer>
			</div>
			); } 
		</div>
