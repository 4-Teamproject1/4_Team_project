		...
		try {
			...
			for (WebElement conferenceElement : conferenceElements) {
				...
				try {
					...
					for (WebElement row : rows) {
						...
						for (int i = 0; i < cells.size(); i += 2) {
							...
							case "접수기간":
								...
								competition.setApplicationPeriod(applicationPeriod); // Assuming a setter method exists
								break;
							case "총시상금":
								totalPrizeMoney = value;
								competition.setTotalPrizeMoney(totalPrizeMoney); // Assuming a setter method exists
								break;
							case "1등시상금":
								firstPrizeMoney = value;
								competition.setFirstPrizeMoney(firstPrizeMoney); // Assuming a setter method exists
								break;
							case "관련 홈페이지":
								WebElement linkElement1 = cells.get(i + 1).findElement(By.tagName("a"));
								homepage = linkElement1.getAttribute("href");
								competition.setHomepage(homepage); // Assuming a setter method exists
								break;
							case "담당자 연락처":
								contactNum = value;
								competition.setContactNum(contactNum); // Assuming a setter method exists
								break;
							case "담당자 이메일":
								contactEmail = value;
								competition.setContactEmail(contactEmail); // Assuming a setter method exists
								break;
							}
						}
					}
					...
					competition.setImageURL(imageURL); // Assuming a setter method exists
					...
					competitions.add(competition); // Add to list of competitions

					System.out.println("공모전 정보: " + competition); // Assuming a toString method in Competition class
